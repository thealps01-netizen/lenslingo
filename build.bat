@echo off
title LensLingo — Build
echo ============================================
echo   LensLingo  Build + Installer
echo ============================================
echo.
...
:: ── Build exe (onedir via LensLingo.spec) ─────────────────────────────────────
echo  [3/4] Building LensLingo.exe...
python -m PyInstaller LensLingo.spec --noconfirm
...
if defined ISCC (
    for /f %%v in ('python -c "from version import __version__; print(__version__)"') do set APP_VER=%%v
    %ISCC% /DAppVersion=%APP_VER% lenslingo.iss
    if errorlevel 1 ( echo  ERROR: Inno Setup failed. & pause & exit /b 1 )
    echo.
    echo  ============================================
    echo   Done!  installer\LensLingo_Setup.exe
    echo  ============================================
    echo.
) else (
    echo.
    echo  Inno Setup not found — skipping installer.
    echo  Portable exe: dist\LensLingo.exe
    echo.
