...
Usage in lenslingo.py:
    from updater import UpdateChecker, prompt_and_install
    checker = UpdateChecker("thealps01-netizen", "lenslingo")   # ← GitHub user/repo
    checker.update_available.connect(on_update)
    checker.start()

GitHub Release convention:
    Tag:   v1.2.3
    Asset: LensLingo_Setup.exe   (the installer)
"""
...
def _fetch_latest(owner: str, repo: str) -> dict | None:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": f"LensLingo-Updater/{__version__}",
        },
    )
    try:
...
class InstallerDownloader(QObject):
    ...
    def _run(self) -> None:
        try:
            suffix  = os.path.basename(self._url.split("?")[0]) or "LensLingo_Setup.exe"
            tmp_dir = tempfile.mkdtemp(prefix="lenslingo_update_")
            ...
            try:
                ...
                with urllib.request.urlopen(
                    urllib.request.Request(sha256_url, headers={"User-Agent": "LensLingo-Updater/1.0"}),
                    timeout=TIMEOUT,
                    context=ctx,
                ) as r:
...
class UpdateDialog(_DraggableDialog):
    ...
    def __init__(self, tag: str, notes: str, parent=None):
        ...
        self.setWindowTitle("LensLingo — Update Available")
        ...
        subtitle = QLabel("An update is available for LensLingo.")
...
class DownloadProgressDialog(_DraggableDialog):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("LensLingo — Downloading")
        self.setWindowFlags(Qt.WindowType.Dialog | Qt.WindowType.FramelessWindowHint)
        self.setAttribute(Qt.WidgetAttribute.WA_TranslucentBackground)
    ...
    def set_installing(self) -> None:
        self.setWindowTitle("LensLingo — Installing")
...
class _ErrorDialog(_DraggableDialog):
    """Styled error dialog matching the LensLingo dark theme."""

    def __init__(self, title: str, body: str, parent=None):
        super().__init__(parent)
        self.setWindowTitle("LensLingo — Update Error")
        self.setWindowFlags(Qt.WindowType.Dialog | Qt.WindowType.FramelessWindowHint)
        self.setAttribute(Qt.WidgetAttribute.WA_TranslucentBackground)
