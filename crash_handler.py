"""crash_handler.py — Global unhandled exception handler for LensLingo.

Intercepts uncaught exceptions, writes a timestamped crash log under
%LOCALAPPDATA%/LensLingo/logs/, and shows a user-friendly dialog with
...
def _write_crash_log(exc_type, exc_value, exc_tb) -> tuple[str, str]:
    ...
    try:
        with open(path, "w", encoding="utf-8") as f:
            f.write(f"LensLingo crash report — {datetime.datetime.now()}\n")
            f.write("=" * 60 + "\n\n")
            f.write(tb)
    except Exception:
        pass
...
def _show_crash_dialog(crash_path: str, tb_text: str) -> None:
    ...
    try:
        ...
        msg = QMessageBox()
        msg.setWindowTitle("LensLingo — Beklenmedik Hata")
...
def install() -> None:
    """Replace sys.excepthook with LensLingo's crash handler."""
